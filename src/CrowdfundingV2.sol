    ...
    UUPSUpgradeable
{
    ...
    IERC20 public USDT_TOKEN; // USDT 代币合约地址（可升级）
    ...
    // 新增：平台费用配置
    uint256 public platformFeeRate; // 平台费用率（基点，100 = 1%）
    ...
    /// @param _platformFeeRate 平台费用率（基点）
    ...
    /// @notice 更新平台费用配置（仅所有者）
    /// @param _platformFeeRate 新的平台费用率（基点）
    /// @param _feeRecipient 新的费用接收地址
    function updatePlatformFee(
        uint256 _platformFeeRate,
        address _feeRecipient
    ) external onlyOwner {
        require(_platformFeeRate <= 1000, "Fee rate too high"); // 最大 10%
        ...
        emit PlatformFeeUpdated(oldFeeRate, _platformFeeRate, _feeRecipient);
    }

    /// @notice 更新 USDT 代币地址（仅所有者）
    /// @param _newUsdtToken 新的 USDT 代币地址
    function updateUsdtToken(address _newUsdtToken) external onlyOwner {
        require(_newUsdtToken != address(0), "Invalid token address");
        USDT_TOKEN = IERC20(_newUsdtToken);
    }
    ...
    function pledge(uint256 _id, uint256 _amount) external nonReentrant {
        ...
        emit Pledged(
            ...
            _amount,
            project.pledged,
            investorAmounts[_id][msg.sender]
        );
    }
    ...
    function claim(uint256 _id) external nonReentrant {
        ...
        // 计算平台费用
        uint256 platformFee = (project.pledged * platformFeeRate) / 10000;
        uint256 creatorAmount = project.pledged - platformFee;

        // 将 USDT 转给项目发起人
        require(
            USDT_TOKEN.transfer(project.creator, creatorAmount),
        ...
        if (platformFee > 0) {
            require(
                USDT_TOKEN.transfer(feeRecipient, platformFee),
                "transfer fee failed"
            );
        }

        emit Claimed(_id, project.creator, creatorAmount, true);
    }
    ...
    function batchRefund(
        ...
        for (uint256 i = 0; i < _investors.length; i++) {
            ...
            if (amount > 0) {
                ...
                // 将 USDT 转回给投资人
                require(
                    USDT_TOKEN.transfer(investor, amount),
                    "transfer failed"
                );
    ...
    function getInvestorCountFromList(
        uint256 _id,
    ...
    /// @param _id 项目 ID
    /// @param _addresses 要检查的地址列表
    function getProjectInvestmentRecords(
        uint256 _id,
        address[] calldata _addresses
    )
        external
    ...
    {
        return (platformFeeRate, feeRecipient);
    }

    /// @notice UUPS 升级授权函数（仅所有者可以升级）
